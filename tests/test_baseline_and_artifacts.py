    ...
    assert (
        baseline.predict(
            ...
            DatabaseSchema(
                "concert",
                (
                    TableSpec("singer", (ColumnSpec("id", "INTEGER"),)),
                    TableSpec("concert", (ColumnSpec("id", "INTEGER"),)),
                ),
            ),
        )
...
def test_template_baseline_extracts_numeric_condition() -> None:
    ...
    assert (
        TemplateBaseline().predict("Which name belongs to a person older than 30?", schema)
...
def test_template_baseline_aggregates_selected_column_with_condition() -> None:
    ...
    assert (
        TemplateBaseline().predict("What is the average salary for people from India?", schema)
...
def test_template_baseline_preserves_question_projection_order() -> None:
    ...
    assert (
        TemplateBaseline().predict("Show name, country, age for every singer.", schema)
...
def test_template_baseline_links_number_alias_and_quoted_value() -> None:
    ...
    assert (
        TemplateBaseline().predict("What team is player number 42 on?", schema)
...
def test_template_baseline_evaluates_and_writes_jsonl(tmp_path: Path) -> None:
    ...
    report_payload = json.loads((report_directory / "evaluation.json").read_text(encoding="utf-8"))
